...
class MainActivity : Activity() {
    ...
    private fun buildUi(): LinearLayout {
        ...
        root.addView(text("SWAPNIL SECURITY LAB", 24f))
        root.addView(text("ROOTLESS SECURITY PLATFORM • v1.4.0", 12f))

        val supported = android.os.Build.SUPPORTED_ABIS.contains("arm64-v8a")
        status = text(
            "ARM64 readiness: ${if (supported) "READY" else "UNSUPPORTED"}",
            14f
        )
        root.addView(status)
        ...
        root.addView(button("INSTALL / VERIFY ROOTLESS ENGINE") { install() })
        root.addView(button("START DEBIAN ARM64 VM") { startVm() })
        root.addView(button("NETWORK DISCOVERY") {
            startActivity(Intent(this, ScannerActivity::class.java))
        })
        root.addView(button("TERMINAL") {
            startActivity(Intent(this, TerminalActivity::class.java))
        })
        root.addView(button("VM SETTINGS") { showInfo() })
        root.addView(
            text(
                "\nModules are intended for systems and networks you own or are authorized to assess.\n\nOpen-source attribution is retained in NOTICE.",
                12f
            )
        )
        return root
    ...
    private fun install() {
        status.text = "Downloading, verifying and unpacking rootless engine…"
        io.execute {
            ...
            runOnUiThread {
                status.text = if (ok) {
                    "ROOTLESS ENGINE READY • checksums verified"
                } else {
                    "INSTALL FAILED • check storage/network"
                }
            }
        }
    }
    ...
    private fun showInfo() {
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "ARM64 QEMU\n2 GB default memory\nUser-mode networking\nAndroid root is not required\n\nBuild 1–4 consolidated in v1.4.0"
            )
